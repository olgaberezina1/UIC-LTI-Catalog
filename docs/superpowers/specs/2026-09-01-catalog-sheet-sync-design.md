...
2. *both* `Available in Blackboard` and `Available in Canvas` are in `{Excluded, Retired, blank}`

Rule 2 reproduces today's catalog exactly. Of the sheet's 112 rows, 39 are Excluded in both
columns, 13 are Retired in both, and one (Coursera) is blank in both — leaving 59 rows and,
after de-duplication, the same 58 tools the site shows today.
